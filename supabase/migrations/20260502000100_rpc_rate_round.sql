-- TICKET-037: rate_round RPC — atomically marks participant ready, creates journal entry,
-- inserts photos, and conditionally reveals the round. Race-free via FOR UPDATE + NOT EXISTS guard.
...
BEGIN
    ...
    -- 3. Photos with server-computed sort_order (0-based, derived from array ordinality)
...
COMMENT ON FUNCTION public.rate_round IS
    'TICKET-037: Atomically marks a round participant ready, creates their journal entry, inserts photos (server-computed sort_order), and conditionally reveals the round. Called from supabase/functions/table-night/index.ts.';
