-- TICKET-037: start_round RPC — atomically creates table_nights row, host participant,
-- attendee participant rows, host journal entry, and host photos in a single transaction.
-- Validates all attendees are table members before any writes.
...
COMMENT ON FUNCTION public.start_round IS
    'TICKET-037: Atomically creates a round, host participant row, attendee participant rows, host journal entry, and host photos in one transaction. Called from supabase/functions/table-night/index.ts.';
