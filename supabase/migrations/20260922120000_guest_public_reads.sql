...
-- public account, visibility <> 'private', rated, >= 20 chars of note) on
-- verified, non-tombstoned restaurants. Nothing here reads tables, wishlists,
-- follows or private entries.
--
...
comment on function public.get_public_reviews_page_guest(uuid, integer, timestamptz, uuid) is
    'TICKET-247 guest reads: public-eligible reviews for one restaurant, keyset paged, '
    'no viewer and therefore no block filter. service_role only (public-browse edge function).';
...
comment on function public.fn_guest_public_review_counts(uuid[]) is
    'TICKET-247 guest reads: public-eligible review count per restaurant. service_role only.';
