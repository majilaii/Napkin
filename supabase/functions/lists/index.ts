 ...
 *   repair_ghost     — creator-scoped list ghost correction (CAS fenced)
...
import { retryTransactionRpc } from "../_shared/dbRetry.ts";
...
async function fetchWritableList(
  ...
  if (list.table_id && (await isTableMember(supabase, list.table_id, userId))) {
    return list;
  }
  return null;
}
...
async function resolveRestaurantId(
  ...
  if (body.restaurant) {
    return await upsertRestaurant(supabase, body.restaurant as RestaurantInput);
  }
  return null;
}
  ...
  try {
    ...
    if (action === "create") {
      ...
      const tableId: string | null =
        typeof body.table_id === "string" && body.table_id
          ? body.table_id
          : null;
    ...
    if (action === "list_mine") {
      ...
      if (listIds.length > 0) {
        ...
        for (const row of (verifiedRows ?? []) as Array<{ list_id: string }>) {
          verifiedCounts.set(
            row.list_id,
            (verifiedCounts.get(row.list_id) ?? 0) + 1,
          );
        }
      }
      ...
      const enriched = await Promise.all(
        (lists ?? []).map(async (list: ListRow) => {
          ...
          return buildMyListSummary({
            ...
            coverRestaurant: normalizeMyListCoverRestaurant(
              firstEntry?.restaurant,
            ),
    ...
    if (action === "save_list") {
      ...
      if (parsedListId.error) {
        return jsonResponse({ error: parsedListId.error }, 400);
      }
    ...
    if (action === "get") {
      ...
      const isMemberOfTableList = !!list.table_id &&
        (await isTableMember(supabase, list.table_id, user.id));
      ...
      // A service-role embed bypasses restaurant RLS. Keep incomplete
      // ghosts private to the user who created that restaurant even when
      // the list itself is visible to a stranger or another Table member.
      const visibleEntries = (entries ?? []).flatMap((entry: any) => {
        const projected = projectListEntryForViewer(entry, user.id);
    ...
    if (action === "add_entries") {
      ...
      if (restaurantIds.length === 0) {
        return jsonResponse({
          error: "restaurant_ids must be a non-empty array",
        }, 400);
      }
      // Dedup the request and keep the edge/schema contract identical.
      const ids = [...new Set(restaurantIds)];
      if (ids.length > 100) {
        return jsonResponse({
          error: "restaurant_ids supports at most 100 unique IDs",
        }, 400);
      }
      ...
      if (ids.some((id) => !uuidPattern.test(id))) {
        return jsonResponse(
          { error: "restaurant_ids must contain UUIDs" },
          400,
        );
      }

      // Alias resolution, authority revalidation, gap allocation, insert,
      // and the authoritative count live in ONE database transaction. This
      // closes the validate→canonicalise→insert FK race in the old path.
      const { data, error } = await retryTransactionRpc(() =>
        supabase.rpc("fn_add_list_entries_canonical", {
          p_actor: user.id,
          p_list_id: list_id,
      ...
      if (error) {
        ...
        if (code === "42501") return jsonResponse({ error: "Not found" }, 404);
        throw error;
      }
    ...
    // ── repair_ghost ─────────────────────────────────────────────────
    // The RPC owns every authorization check and row lock. The explicit
    // actor is required because this function connects with service_role,
    // so auth.uid() is intentionally null inside PostgreSQL.
    if (action === "repair_ghost") {
      ...
      const replacementExternalId =
        typeof body.replacement_external_id === "string"
          ? body.replacement_external_id.trim()
          : "";
      const expectedVersion = Number(body.expected_version);
      if (
        !entryId || !listId || !replacementExternalId ||
        !Number.isInteger(expectedVersion)
      ) {
        return jsonResponse({
          error:
            "entry_id, list_id, replacement_external_id, and expected_version are required",
        }, 400);
      }

      const { data: restaurantId, error: repairErr } = await supabase.rpc(
        "fn_repair_list_ghost",
        {
          p_actor: user.id,
          p_entry_id: entryId,
          p_list_id: listId,
          p_replacement_external_id: replacementExternalId,
          p_expected_version: expectedVersion,
        },
      );
      if (repairErr) {
        const code = (repairErr as { code?: string }).code ?? "";
        if (code === "42501") return jsonResponse({ error: "Forbidden" }, 403);
        if (code === "40001") {
          return jsonResponse({
            error: "Restaurant changed; refresh and retry",
          }, 409);
        }
        throw repairErr;
      }
      ...
      return jsonResponse({
        data: { restaurant_id: restaurantId, entry_id: entryId },
      });
    }
    ...
    if (action === "remove_entry") {
      ...
      if (!list_id || !restaurant_id) {
        return jsonResponse(
          { error: "list_id and restaurant_id are required" },
          400,
        );
      }
    ...
    if (action === "update_entry") {
      ...
      if (!list_id || !entry_id) {
        return jsonResponse(
          { error: "list_id and entry_id are required" },
          400,
        );
      }
      ...
      const noteVal = note !== undefined
        ? note ? String(note).slice(0, 140) : null
        : undefined;
    ...
    if (action === "reorder_entry") {
      const {
        ...
        entry_id,
        new_index,
        before_entry_id: legacyBefore,
        after_entry_id: legacyAfter,
      } = body;
      if (!list_id || !entry_id) {
        return jsonResponse(
          { error: "list_id and entry_id are required" },
      ...
      if (typeof new_index === "number") {
        ...
        const withoutMoved = (allEntries ?? []).filter((
          e: { id: string; position: number },
        ) => e.id !== entry_id);
        const clampedIndex = Math.max(
          0,
          Math.min(new_index, withoutMoved.length),
        );
        const beforeEntry = clampedIndex > 0
          ? withoutMoved[clampedIndex - 1]
          : null;
        const afterEntry = clampedIndex < withoutMoved.length
          ? withoutMoved[clampedIndex]
          : null;
        ...
        const neighbourIds = [legacyBefore, legacyAfter].filter(
          Boolean,
        ) as string[];
        ...
        if (fetchErr) throw fetchErr;
        const byId = new Map(
          (neighbours as Array<{ id: string; position: number }>).map((
            e,
          ) => [e.id, e.position]),
        );
        ...
        return jsonResponse({
          error: "new_index or before_entry_id/after_entry_id is required",
        }, 400);
      }
      ...
      const gapTooSmall = prevPos !== null &&
        nextPos !== null &&
        nextPos - prevPos < 2;
    ...
    if (action === "map_pins") {
      ...
      const listMeta = new Map<
        string,
        { emoji: string | null; updated_at: string }
      >();
      for (
        const l of (myLists ?? []) as Array<
          { id: string; emoji: string | null; updated_at: string }
        >
      ) {
        listMeta.set(l.id, {
          emoji: l.emoji ?? null,
      ...
      const pins = [];
      for (
        const row of (rows ?? []) as Array<{ list_id: string; restaurant: any }>
      ) {
    ...
    if (action === "browse_public") {
      const BROWSE_CAP = 6;
      const { data: rpcRows, error: rpcErr } = await supabase.rpc(
        "fn_browse_public_lists_with_cover_credit",
        {
          p_viewer: user.id,
          p_limit: BROWSE_CAP,
        },
