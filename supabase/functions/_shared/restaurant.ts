...
import { hasCompleteRestaurantFacts } from "./completeness.ts";
...
export async function upsertRestaurant(
  ...
  if (
    input.googleRatingCount !== undefined && input.googleRatingCount !== null
  ) {
  ...
  if (input.latitude !== undefined && input.latitude !== null) {
    upsertRow.lat = input.latitude;
  }
  if (input.longitude !== undefined && input.longitude !== null) {
    upsertRow.lng = input.longitude;
  }
  ...
  const wroteAnyPlacesField = Object.keys(placesMetadata).length > 0 ||
    !!phone ||
    !!website ||
    !!googleMapsUri ||
  ...
  if (input.verification !== undefined) {
    upsertRow.verification = input.verification;
  }
  ...
  if (input.photoReference && !data.photo_url) {
    await _storeHeroPhoto(
      supabase,
      data.id,
      input.photoReference,
      input.photoAttributionHtml ?? null,
    );
  }
...
async function _storeHeroPhoto(
  ...
  try {
    ...
    if (!photoAttributionHtml || photoAttributionHtml.trim() === "") {
      console.log(
        `No attribution for ${restaurantId} — stamping photo_source='none', skipping mirror.`,
      );
      await supabase
        .from("restaurants")
    ...
    const mediaUrl =
      `https://places.googleapis.com/v1/${photoReference}/media?maxHeightPx=800&maxWidthPx=1200&key=${apiKey}`;
    const mediaRes = await fetch(mediaUrl);
    if (!mediaRes.ok) {
      console.error(
        `Photo fetch failed for ${restaurantId}: HTTP ${mediaRes.status}`,
      );
      return;
    }
...
export function buildPhotoAttributionHtml(body: unknown): string | null {
  ...
  const displayName =
    typeof att?.displayName === "string" && att.displayName.trim() !== ""
      ? att.displayName
      : null;
  if (!displayName) return null;
  const uri = typeof att?.uri === "string" && att.uri.trim() !== ""
    ? att.uri
    : null;
  return uri
 ...
 *
 * TICKET-195 routes this compatibility entry point through the shared provider:
 * attestation cache → winner-only Details debit → media claim → winner-only
 * Photo debit → reference-derived Storage object → exact-URL canonical CAS.
 * Repeat-complete rows are skipped before a claim and all transient outcomes
 * remain retryable by the durable queue/backfill.
 */
export async function acquireAndMirrorHeroPhotos(
  ...
  try {
    ...
    const { data: rows, error } = await supabase
      .from("restaurants")
      .select(
        "id, external_id, lat, lng, photo_url, photo_source, places_photo_attribution_html, verification, merged_into",
      )
      .in("id", ids);
    if (error) {
      ...
      reportError(error, {
        ...
        extra: {
          user_id: userId,
          stage: "restaurants_read",
          restaurant_count: ids.length,
        },
      });
      return;
    }
    ...
    for (const row of rows) {
      ...
      if (!row.external_id || row.verification !== "verified") continue;
      if (hasCompleteRestaurantFacts(row)) continue;
      ...
      try {
        ...
        if (providerError instanceof CompletenessPaidPathError) {
          // Budget/claim contention and provider failures remain
          // retryable. They are ordinary lifecycle states, not a
          // reason to crash the fire-and-forget batch.
          if (providerError.code === "BUDGET_DEFERRED") return;
          continue;
        }
