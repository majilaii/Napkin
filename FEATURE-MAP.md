...
- Persisted history failure with cached identity retains the page and shows “could not load visit history” with retry. Failure before identity exists shows a dedicated “could not load this restaurant” state with both back and retry; it never paints blank paper. Ghost lookup failure can only degrade to the route's available identity parameters; do not mistake missing optional history for a hard crash.
...
- The Napkin distribution and both photo rails are viewer-specific. `restaurant-history` batches every non-self candidate through the canonical entry-visibility branches with a bidirectional block gate: strangers cannot see a private account's palate, while unblocked Tablemates, companions, and Supper members retain their scoped access.
...
| Restaurant Napkin count or photo rails differ by viewer | This is privacy filtering, not missing data. The service-role `restaurant-history` endpoint batches aggregate and photo candidates through `fn_visible_entry_ids`; private-account strangers and either-direction blocks are withheld, while authorized Table/companion/Supper scopes remain. | `supabase/functions/restaurant-history/index.ts`; `supabase/migrations/20260826155643_ticket_217_restaurant_privacy_gates.sql` |
