...
- THE REGULAR (`RestaurantRegularRow`: kicker, 40pt avatar, upright name, `N visits · <runner-up> is N behind` / `tied with`) follows the masthead ledger/actions/memories group but is not inside the ledger's self-history gate: a followee can hold the crown when the viewer has no visit here. Since 2026-09-06 the restaurant-page crown is ALL TIME and counts every visible check-in, rated or not, dated or not (`loadRestaurantRegular`, read category `regular`, minimum two visits; the monthly ledger crown keeps its own rolling-90-day rated window). It is hidden when `regular_detail` is null; crown-read failures are non-fatal and suppress only this row, never the core restaurant page. Cohort = viewer + followees only, filtered before the shared visibility RPC, so public strangers and unauthorized private logs never hold or affect the crown while authorized Table/companion/Supper rows can. A followee's row opens `/u/[identifier]`; the viewer's own crown (`you`) is inert.
