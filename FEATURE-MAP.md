...
- YOU comes from `personal.average` plus `self_log.length`; FRIENDS comes only from followee `public_reviews` deduped by author; GOOGLE remains a labelled external 0–5 value. The friend spread derives from the same followee cohort and is intended-empty below three people.
- `FROM FRIENDS` renders at most two followee public notes and opens `/restaurant-reviews`; `FROM <TABLE>` renders at most two rows from one authorized `table_notes` share-edge group and opens `/(tabs)/tables?selected=&section=activity`. Those rings never cross-feed.
- The history doorway is intended-empty when `self_log` is empty or the restaurant is a ghost. Warm scoped and unscoped arrivals reuse the exact `restaurants.page(id, tableId?)` cache; a cold `/restaurant-history?id=` deep link shows loading, then the ledger. Query error without cache is broken-empty (`ErrorState` + retry).
- `/restaurant-history` renders every viewer-authored row, including unrated and non-host Round takes. Zero rows is an intended empty invitation. `ELSEWHERE` is intended-empty unless a pin, containing list, or self clipping exists. It is read-only: card navigation and list doorways mutate nothing.
- Featured lists, social clips, Table atlas context, hours, phone, and website are conditional real-data sections. Their absence after successful reads is intended-empty. The Tables `section=activity` param re-applies on each param instance; omitting `section` preserves the current pane.
