...
- There are **70 default-export `.tsx` modules** under `app/`: five layouts plus **65 route modules** (64 intended pages and the accidentally routable `/onboarding/OnboardingProgress` support component). This count was re-checked (`grep -L "export default"` against every `app/**/*.tsx`) after the two retired map routes were removed and `/places-scope` was added.
...
- A quick check-in (`Check in` / `Check in again`, footsteps icon) is dated now by `fn_record_visit` (founder order 2026-09-06; migration `20260906120000_dated_quick_visits.sql`), with no rating or text and friends visibility. The plate shows that date as a tappable link so a retrospective visit can be moved (`Today` / `Yesterday` / `Another day` / `No date`). Intentional repeats create distinct entries; retries retain the nonce. Add a review updates the selected entry ID with optional half-stars, words and photos. Photos stage on Save and bind atomically with the review. Cancel discards drafts. Existing full `Log this meal` remains available.
- The visit plate (`Checked in · N visits`) opens history, ordered by recording time and ID; changing a date does not renumber visits. Dates can still be cleared, and undated legacy history says `no date`. `is_bare` no longer requires a null date, so `undo` stays offered for the newly recorded check-in whether or not its date was moved; a review, photo, share or companion still ends it. All-time counts include bare visits, while averages use rated entries and calendar-year/month/90-day awards require a known visit date. Unknown dates cannot join a same-night merge. Existing diary keysets retain their ordering fallback without turning it into a displayed visit date.
- `/restaurant-reviews` retains `useRestaurantReviews` and `restaurant-history?action=reviews` / `get_public_reviews_page`. The EF batch-loads `entry_photos` only for eligible returned review IDs, adding `photo_urls` with legacy `photo_url` fallback; `action=page` runs the same `loadReviewPhotos` over its capped `public_reviews` (non-fatal, falling back to the RPC's single `photo_url`). Light rows display text and 1–3 inline photos, plus overflow count, through `components/restaurants/ReviewPhotoStrip.tsx`; photos open read-only `PhotoLightbox`. Long text expands in place. Public eligibility and pagination are unchanged.
- States: initial loading disables visit controls; no visits shows Check in; existing bare visit offers Add a review; reviewed visit shows its rating on the plate with an explicit `edit` link; the note and photos stay in the visit itself and never repeat on the plate. History/date/editor sheets have explicit close controls. Network/moderation/save failures retain the draft and show an error; uncertain record failure offers Retry visit. All reviews preserves initial loading/error/empty and subsequent-page retry states. Own bare Feed rows say `been to` among peer activity.

| Control | Refusal conditions | Live verification |
|---|---|---|
| Check in / Check in again / Retry check-in | Signed out, request pending, no restaurant identity; server rejects invalid/reused nonce for another restaurant | WRITE — test fixtures only |
...
| undo | Only offered for the newly recorded bare visit; server refuses anything except the latest owned, unshared and unenriched entry (a date is metadata, not enrichment) | DELETE — isolated SQL/component fixtures only |
...
- The action rail is the visit plate plus the two visit buttons (`Check in`/`Check in again`, `Log this meal`) and nothing else. Pinning is the masthead bookmark only. Directions (address row action), website, `reserve a table` (only when `useReserveLink` resolves a real booking page) and `gather the table` (Table membership plus a persisted restaurant) are all icon rows in DETAILS, in that order before the inert Google row.
- The reviews section previews up to three public review cards (`previewReviews`: followee cards first, then other public reviews, never the viewer's own, deduped by entry); each note clips at three lines, the review's photos render inline beneath it through the shared `ReviewPhotoStrip` (1–3 tiles, `+N` overflow on the third, taps open the read-only `PhotoLightbox`), and the `all N reviews ›` doorway below opens `/restaurant-reviews`. The kicker reads `FROM FRIENDS` only when every previewed card is a followee's, otherwise `REVIEWS`. With a public total but no previewable card it collapses to the single `REVIEWS · all N reviews` doorway row. `FROM <TABLE>` renders at most two rows from one authorized `table_notes` share-edge group and opens `/(tabs)/tables?selected=&section=activity`. Table-note authors must be current members of that exact shared Table; the rings never cross-feed.
...
Source: `app/(tabs)/places.tsx`, `app/places-scope.tsx`, `components/places/PlacesScreen.tsx`, `components/places/ScopePickerSheet.tsx`, `components/places/TableScopeRow.tsx`, `components/wishlist/WishlistMapView.tsx`, `components/wishlist/ClipTray.tsx`, `components/wishlist/clipTrayUtils.ts`, `components/sheets/SnapSheet.tsx`, `components/places/PlacesRow.tsx`, `components/places/PlacesSearchSections.tsx`, `components/places/PlacesListsPane.tsx`, `components/places/placesPresentation.ts`, `components/search/SearchLocalityBar.tsx`, `components/search/PeopleSearchPane.tsx`, `components/search/ListsSearchPane.tsx`, `components/search/RecentSearchesList.tsx`, `components/search/ListRow.tsx`, `hooks/lists/useMyLists.ts`, `hooks/lists/useSavedLists.ts`, `hooks/users/useFollowingList.ts`, `hooks/users/useNetworkMapPins.ts`, `hooks/wishlist/useTableWishlist.ts`, `hooks/tables/useTableMapPins.ts`, `hooks/tables/useTableMembers.ts`, `hooks/search/useRestaurantSearch.ts`, `hooks/search/placesScreenState.ts`, `hooks/search/useSearchLocality.ts`, `providers/AuthProvider.tsx`.
