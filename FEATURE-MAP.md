...
| Retired wishlist or Table-map link opens no screen | The two legacy route modules were intentionally deleted when Places became the single map. All old wishlist destinations must target `/(tabs)/places?view=list&layer=pinned`; Table map controls must push `/places-scope?scope=table&tableId=…`. | TICKET-237; `napkin-app/lib/handoffNavigation.ts`; `napkin-app/app/(tabs)/tables.tsx`; `napkin-app/app/places-scope.tsx` |
