...
- The memories strip sits between the masthead and numbers for persisted restaurants only. It reads no new source: own `self_log[].photos` newest-first, then `photos.from_your_table`, `photos.from_others`, and `public_reviews[].photo_url`; duplicate URLs are removed and the first 12 remain. Ghosts, zero authorized photos, and legacy payloads without the `photos` projection render byte-for-byte no section or spacer. Entry-backed tiles open entry detail; projection rows without an entry id stay inert.
...
Source: `app/(tabs)/places.tsx`, `components/wishlist/WishlistMapView.tsx`, `components/wishlist/ClipTray.tsx`, `components/wishlist/clipTrayUtils.ts`, `components/sheets/SnapSheet.tsx`, `components/places/PlacesRow.tsx`, `components/places/PlacesSearchSections.tsx`, `components/places/PlacesListsPane.tsx`, `components/places/placesPresentation.ts`, `components/search/SearchLocalityBar.tsx`, `components/search/PeopleSearchPane.tsx`, `components/search/ListsSearchPane.tsx`, `components/search/RecentSearchesList.tsx`, `components/search/ListRow.tsx`, `hooks/lists/useMyLists.ts`, `hooks/lists/useSavedLists.ts`, `hooks/users/useFollowingList.ts`, `hooks/users/useNetworkMapPins.ts`, `hooks/search/useRestaurantSearch.ts`, `hooks/search/placesScreenState.ts`, `hooks/search/useSearchLocality.ts`, `providers/AuthProvider.tsx`.
...
- **Pagination / repair / share:** the all and pinned browse ledgers page the 40-row wishlist cursor on end reached and show a `+` on the last group until exhaustion; been, friends, Lists, People, and every query never request the next wishlist page. Coordinate-less loaded pins surface the existing repair murmur/sheet only under all or pinned. The pinned-only share icon is map-mode only and opens the unscoped wishlist handoff with the unfiltered loaded pinned total even when facets narrow the visible ledger.
...
| Restaurant detail has no map/Places-photo hero | Deliberate v3 grammar: the page opens on plain top bar + upright masthead; only already-authorized visit memories may appear in the dedicated strip. | `napkin-app/app/restaurant/[id].tsx`; `napkin-app/components/restaurants/MemoriesStrip.tsx`; `napkin-app/components/restaurants/RestaurantPageV3.tsx` |
| Restaurant has three ratings that do not combine | YOU, FRIENDS, and labelled GOOGLE are separate cells. Google is never input to a Napkin aggregate; Table notes are not FRIENDS. | `napkin-app/lib/restaurantPageV3.ts`; `napkin-app/components/restaurants/RestaurantPageV3.tsx` |
