...
- Clip tray video and screenshot actions open the matching native Photos picker directly. `ImportLinkSheet` presents an empty transparent host without animation or another scrim, waits for its native `onShow`, then opens Photos; the import-source menu never paints during this handoff. Cancel returns to the tray, and reopening starts a fresh picker. Selection reveals the existing extraction/upload state. Picker presentation failures show an inline retry; dismissed or replaced picker/upload results cannot start resolution. Opening and cancelling Photos is read-only; selecting media, resolving, or saving stays fixture-only during verification.
