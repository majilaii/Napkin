...
- The action rail is the two visit buttons (`Been here`/`Been again`, `Log this meal`/`Add a review`) over one quiet text row `pin · directions · website · reserve` (`pinned` reads terracotta; website/reserve appear only when resolved). Gather is not on the rail: it lives in DETAILS as the `gather the table` row, shown only with a Table membership and a persisted restaurant.
