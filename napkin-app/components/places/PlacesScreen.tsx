...
import {
    ...
    type PlacesLayerFilter,
    type PlacesScope,
    type PlacesScreenStateStore,
    type PlacesViewMode,
...
function routeView(view: string | undefined): PlacesViewMode | null {
    return view === 'list' || view === 'map' ? view : null;
}

function routeLayer(layer: string | undefined): Exclude<PlacesLayerFilter, 'all'> | null {
    return layer === 'pinned' || layer === 'been' ? layer : null;
}

function routeScope(scope: string | undefined): PlacesScope | null {
    return scope === 'friends' ? { kind: 'friends' } : null;
}
...
const ActiveTableWhoChip = React.memo(function ActiveTableWhoChip({
    tableId,
    tableName,
    palette,
    onPress,
    ...
    const { data } = useTableMembers(tableId);
    return (
        <WhoChip
...
interface PlacesScreenProps {
    stateStore?: PlacesScreenStateStore;
    lockedScope?: PlacesScope;
    onBack?: () => void;
...
export function PlacesScreen({
    ...
    const { value: screenState, patch: patchScreenState } = usePlacesScreenState(
        user?.id,
        stateStore,
    );
    const activeScope = lockedScope ?? screenState.scope;
    ...
    const requestedView = routeView(incomingView);
    const requestedLayer = routeLayer(incomingLayer);
    const requestedScope = lockedScope ? null : routeScope(incomingScope);
    const routeWantsSearch = incomingQ !== undefined || incomingMode !== undefined;
    // Locked instances (app/places-scope.tsx) own their scope entirely
    // through the `lockedScope` prop — the route's own `scope`/`tableId`
    // params are read once by the parent wrapper to build that prop and must
    // never be touched again. Treating them as a route request here would
    // call `router.setParams` to clear `scope`, which also clears the
    // parent's `useLocalSearchParams` read and collapses `lockedScope` back
    // to `{ kind: 'you' }` (TICKET-237 fix pass 1, P1).
    const hasRouteRequest = !lockedScope && (
        routeWantsSearch
    ...
    useEffect(() => {
        ...
        // A retired-wishlist landing (`?view=list&layer=pinned`, no `mode`/`q`)
        // must land on the Places pane in list mode, out of search — even
        // when the tab was left on Lists/People or mid-search.
        const wishlistLandingRequest = !routeWantsSearch
            && (requestedView !== null || requestedLayer !== null);
        let nextState = routeWantsSearch
            ? enterPlacesSearch(screenState)
            : wishlistLandingRequest
              ? leavePlacesSearch(screenState)
              : screenState;
        if (requestedMode) {
        ...
        } else if (wishlistLandingRequest) {
            nextState = transitionPlacesSegment(
                nextState,
                'places',
                FRIEND_TEST.hidePeopleSearch,
        ...
        if (requestedView) nextState = { ...nextState, viewMode: requestedView };
        if (requestedLayer) nextState = { ...nextState, layerFilter: requestedLayer };
        if (requestedScope) nextState = { ...nextState, scope: requestedScope };
    ...
    const tablesQuery = useTables(user?.id);
    const requestedTableId = activeScope.kind === 'table' ? activeScope.tableId : null;
    const activeTable = (tablesQuery.data ?? [])
        .map((membership) => membership.tables)
        .find((table) => table?.id === requestedTableId) ?? null;
    const activeTableId = activeTable?.id ?? null;
    const tableName = activeTable?.name ?? 'the table';
    const tableWishlistQuery = useTableWishlist(user?.id, activeTableId);
    const tableMapPinsQuery = useTableMapPins(activeTableId, {
        enabled: activeScope.kind === 'table' && screenState.layerFilter === 'been',
    });
    ...
    const networkQuery = useNetworkMapPins(user?.id, {
        enabled: activeScope.kind === 'friends',
    });
    ...
    const tablePinnedRows = useMemo(
        () => tableWishlistRowsToDisplayRows(tableWishlistQuery.data ?? [], {
            tableId: activeTableId ?? '',
            tableName,
        }),
        [activeTableId, tableName, tableWishlistQuery.data],
    );
    const tableBeenRows = useMemo(
        () => tableMapPinsToDisplayRows(tableMapPinsQuery.data ?? []),
        [tableMapPinsQuery.data],
    );
    ...
    const layerLoading = activeScope.kind === 'friends'
        ? networkQuery.isLoading
        : activeScope.kind === 'table'
          ? tablesQuery.isLoading || (screenState.layerFilter === 'been'
            ? tableMapPinsQuery.isLoading
            : tableWishlistQuery.isLoading)
          : screenState.layerFilter === 'all'
    ...
    const failurePresentation = resolvePlacesFailurePresentation({
        ...
        tableWishlistFailed: tablesQuery.isError || tableWishlistQuery.isError,
        tableMapFailed: tablesQuery.isError || tableMapPinsQuery.isError,
    });
    ...
    const unmappableCount = activeScope.kind === 'you'
        && !searchMode
        && immediateQuery.trim().length === 0
    ...
    const navigationClearance = hasBottomNav ? NAV_CLEARANCE : Spacing.md;
    const listModeBottomPadding = insets.bottom
        + navigationClearance
        + VIEW_TOGGLE_HEIGHT
        + Spacing.md;
    const currentPins = useMemo(() => projectPlacesPins(filteredRows), [filteredRows]);
    const scopeKey = activeScope.kind === 'table'
        ? `table:${activeScope.tableId}`
        : activeScope.kind;
    // Facets are drawn from the active layer's rows: a price facet chosen
    // under table-pinned would otherwise hide every gathering once the
    // layer switches to `been` (been rows carry no price). Reset on a scope
    // change and on any transition into `been`.
    useEffect(() => {
    ...
    useEffect(() => {
        if (screenState.layerFilter !== 'been') return;
        setCuisineFilter(null);
        setPriceFilter(null);
        setCityFilter(null);
    }, [screenState.layerFilter]);
    const currentScopeKey = queryActive
        ...
        : `scope:${scopeKey}:layer:${screenState.layerFilter}`;
    ...
    const handleRetryFailure = useCallback(() => {
        for (const source of failurePresentation.sources) {
            ...
            if (source === 'tableWishlist') {
                void tablesQuery.refetch();
                void tableWishlistQuery.refetch();
            }
            if (source === 'tableMap') {
                void tablesQuery.refetch();
                void tableMapPinsQuery.refetch();
            }
        }
    }, [
    ...
    }, [activeSegment, paginatedBrowse, screenState.layerFilter, searchMode, wishlistQuery]);

    const handleScopeSelect = useCallback((scope: PlacesScope) => {
        // A new WHO is a fresh view of that scope: the layer chip resets to all.
        patchScreenState({ scope, layerFilter: 'all', selectedPinId: null, scrollOffset: 0 });
    }, [patchScreenState]);
    const handleOpenScopePicker = useCallback(() => setScopePickerOpen(true), []);
    ...
    const renderBrowseRow = useCallback(({ item }: { item: DecoratedPlacesRow }) => {
        const tableScope = item.row.tableScope;
        if (tableScope?.kind === 'pinned') {
            return (
                <TableScopeRow
                    kind="pinned"
                    item={tableScope.item}
                    palette={palette}
                    onPress={() => openRestaurant(item.row)}
                />
            );
        }
        if (tableScope?.kind === 'been') {
            return (
                <TableScopeRow
                    kind="been"
                    item={tableScope.item}
        ...
        return <PlacesRow item={item} onPress={openRestaurant} />;
    }, [openRestaurant, palette]);

    const browseEmptyMessage = activeScope.kind === 'friends'
        ? screenState.layerFilter === 'pinned'
            ? 'no pinned places from friends yet'
        ...
        : activeScope.kind === 'table'
          ...
          : screenState.layerFilter === 'pinned'
            ...
            : screenState.layerFilter === 'been'
              ...
              : 'no places yet';
    const ownProfile = ownProfileResult?.data?.profile;
    const followingCount = ownProfileResult?.data?.stats?.following_count
        ?? ownProfileResult?.data?.social?.following_count;
    const pickerPinnedCount = wishlistData && !wishlistQuery.hasNextPage
        ? loadedWishlistItems.filter((item) => !!item.restaurant).length
        : undefined;
    const pickerBeenCount = spotsData ? spotsData.length : undefined;
        ...
        if (searchMode && searchGuidanceBranch === 'sections') {
            ...
            return (
                <PlacesSearchSections
                    ...
                    bottomPadding={insets.bottom + navigationClearance}
                />
            );
        }
            ...
            return (
                <Animated.FlatList<PlacesCityLedgerItem>
                    ...
                    renderItem={({ item }) => item.kind === 'header' ? (
                        ...
                        renderBrowseRow({ item: item.item })
                    )}
                    keyboardShouldPersistTaps="handled"
                    keyboardDismissMode="on-drag"
        ...
        return (
            <Animated.FlatList
                ...
                contentContainerStyle={[
                    ...
                    { paddingBottom: insets.bottom + navigationClearance },
                ]}
                ListHeaderComponent={failurePresentation.kind === 'inline'
                    ? (
                        <InlineErrorState
        ...
        <View style={[styles.screen, { backgroundColor: palette.background }]}>
            {mapMode ? (
                <WishlistMapView
                    ...
                    unmappableLabel={`${unmappableCount} pinned ${unmappableCount === 1 ? 'spot has' : 'spots have'} no map location`}
            ...
            >
                ...
                >
                    ...
                    {!searchMode ? (
                        <>
                            {activeScope.kind === 'table' && activeTableId && !scopePickerOpen ? (
                                <ActiveTableWhoChip
                                    tableId={activeTableId}
                                    tableName={tableName}
                                    palette={palette}
                                    onPress={lockedScope ? undefined : handleOpenScopePicker}
                                />
                            ) : (
                                <WhoChip
                                    scope={activeScope}
            ...
            {!lockedScope ? (
                <ScopePickerSheet
                    ...
                    pinnedCount={pickerPinnedCount}
                    beenCount={pickerBeenCount}
                    followingCount={followingCount}
                    tables={tablesQuery.data ?? []}
                    onSelect={handleScopeSelect}
                    onDismiss={() => setScopePickerOpen(false)}
                />
            ) : null}
        </View>
    );
...
const styles = StyleSheet.create({
    ...
    backButton: {
        width: Spacing.hitTarget - Spacing.xs,
        height: Spacing.hitTarget - Spacing.xs,
