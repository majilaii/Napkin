...
interface PlacesCreditProps {
    credits: readonly (PlacesPhotoCredit | null | undefined)[];
    /** Number of rendered Places photos, before author dedupe. */
    photoCount?: number;
...
/**
 * The only visual treatment for Google Places photo attribution.
 * Keep this adjacent to the image block or in the owning surface header —
 * never position it over image pixels.
 */
export function PlacesCredit({
    ...
    const authors = dedupePlacesCredits(credits);
    if (authors.length === 0) return null;

    const renderedPhotoCount = photoCount ?? credits.filter(Boolean).length;
    const prefix = renderedPhotoCount > 1 ? 'photos' : 'photo';
    ...
    return (
        <Text
            ...
            style={[
                ...
                authors.every((author) => author.redundant) && styles.redundantLine,
                style,
            ]}
        >
            {prefix}
            ...
            {authors.map((author, index) => (
                <React.Fragment key={author.normalizedLabel}>
                    {index > 0 ? ', ' : null}
                    <Text
                        ...
                        style={[
                            interactive && author.href && !author.redundant && styles.link,
                            author.redundant && styles.redundantAuthor,
                        ]}
                    >
                        {author.label}
                    </Text>
