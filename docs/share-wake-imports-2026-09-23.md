...
What iOS allows: an extension cannot run the import (about 120 MB of memory, killed when the sheet closes) and cannot start the app directly. It can start a background URLSession transfer; when that finishes, iOS launches or resumes the containing app in the background and calls `application(_:handleEventsForBackgroundURLSession:completionHandler:)` (App Extension Programming Guide, "Performing Uploads and Downloads"). That launch is the wake-up, with one catch that the adversarial review found: if the transfer completes while the extension is still running, the callback goes to the extension and the app is never launched (Apple DTS, forums thread 76659). A tiny upload to a fast server nearly always finishes inside the extension's last second on screen, so PR #387's upload rarely woke anything. The server therefore holds its answer to an extension's upload for eight seconds, long enough for the extension to be torn down. iOS gives the app roughly 30 seconds of background time per wake; a later background download finishing wakes it again. A force-quit app is not relaunched.
...
- `background-imports` hands every enqueued job to the device at once, so installed builds stop waiting on the server, and its worker only hands leftover jobs to their device; a new intake-only `wake` action authenticates and stores nothing. Extension uploads are answered after eight seconds so the wake reaches the app.
