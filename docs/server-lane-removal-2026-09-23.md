...
2. Direct SQL and RPCs: in SQL, only the two creating migrations (`20260913201042`, `20260913200921`) and the two specs. Live callers after this PR are two edge functions. `background-imports/index.ts` calls `fn_enqueue_background_import` and updates `background_import_jobs` in `enqueue`, updates and reads it in `status`, reads it in `list`, and calls `fn_dismiss_background_import` in `dismiss`; the phase C release removes or stubs each of these. `notifications/importPushDevice.ts` calls `fn_register_import_push_device` and `fn_revoke_import_push_device`; the phase B release stubs both. `fn_claim_background_imports`, `fn_finish_background_import` and `fn_claim_import_push_deliveries` have no caller after this PR. `check_and_increment_rate_limit` keeps stale `background_import` and `background_import_dismiss` bucket rows, which are harmless; `import_intake_registration` stays live with `register_intake`.
